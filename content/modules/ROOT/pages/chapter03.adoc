...
* To access *SonarQube*, you use the following link: https://sonarqube-sonarqube.{OPENSHIFT_CLUSTER_INGRESS_DOMAIN} and log in with your credentials, username: {adminuser} and password: {password}.
...
* To access *Trusted Profile Analyzer (TPA)*, you use the following link: https://sso-trusted-profile-analyzer.{OPENSHIFT_CLUSTER_INGRESS_DOMAIN} and log in with your credentials, username: {adminuser} and password: {password}.
...
* You execute the command in the terminal:
+
[source, role="execute"]
----
